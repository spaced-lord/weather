...
for (var i = 0; i < cityArray.length; i++) {
  var pastBtn = $("<button>").attr("class", "btn btn-outline-primary btnsize");
  pastBtn.text(cityArray[i]);
  $(".searchHistory").prepend(pastBtn);
}

var todayHeader = $("<h2>").attr("style", "float_left");
var currentDate = "";
var currentDayImg = "";
var endFloat = $("<h4>").attr("style", "clear:both");
var currentHumidity = "";
var currentWind = "";
var uvIndexEl = $("<h4>").text("UV Index: ");
var uvSpanEl = $("<span>");
var fiveDay = $("<h1>").text("5 Day Forecast:").attr("class", "row m-2");
var dailyForecast = $("<div>").attr("class", "row m-2");
var dailyCard = [];
var dailyCardBody = [];
var forecastDate = [];
...
function formatFunction() {
  ...
  $(".currentData").append(todayHeader);
  ...
  $(".currentData").append(
    $("<h4>").text("Wind Speed: " + currentWind + " MPH")
  );
  $(".currentData").append(uvIndexEle);
  uvIndexEle.append(uvSpanElement);
  $(".col-10").append(fiveDay);
  $(".col-10").append(dailyForcast);
  for (var i = 1; i < 6; i++) {
